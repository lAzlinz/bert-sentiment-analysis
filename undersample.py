...
with open('./datasets/train_data.csv', encoding='utf-8', mode='r') as f:
    csv_reader = csv.reader(f, delimiter=',')
    next(csv_reader)
    labels = ['headline', 'sentiment']
...
def resampler(class_label):
    return resample(class_label, replace=False, n_samples=33_333, random_state=random_state)
...
undersampled_df.to_csv('./datasets/99k_balanced_dataset_with_sentiment.csv', index=False)
...
# train_df.to_csv('99_999_balanced_train_dataset_with_sentiment_0.49.csv', index=False)
# val_df.to_csv('99_999_balanced_val_dataset_with_sentiment_0.21.csv', index=False)
# test_df.to_csv('99_999_balanced_test_dataset_with_sentiment_0.30.csv', index=False)